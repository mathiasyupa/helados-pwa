...
export default function AdminPage() {
  ...
  async function handleGrant() {
    ...
    if (res.ok) {
      ...
      router.push(`/admin/grant-qr?url=${encodeURIComponent(data.url)}&stamps=${data.stamps}&exp=${data.expiresInSeconds}`);
    }
    setGranting(false);
  }
        ...
        <div className="bg-white rounded-2xl p-5 card-shadow">
          ...
          <button
            ...
            className="w-full gradient-btn text-white font-bold py-3 rounded-xl
                       disabled:opacity-50 active:scale-95 transition-all"
          >
            {granting ? 'Generando...' : `📱 Mostrar QR de ${grantQty} ${grantQty === 1 ? 'sello' : 'sellos'}`}
          </button>
        </div>
